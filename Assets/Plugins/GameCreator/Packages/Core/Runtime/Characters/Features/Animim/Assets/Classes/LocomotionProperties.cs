...
namespace GameCreator.Runtime.Characters
{
    ...
    public class LocomotionProperties
    {
        ...
        [NonSerialized] private Dictionary<int, Guid> m_RunningLayers = new Dictionary<int, Guid>();
        
        // PUBLIC METHODS: ------------------------------------------------------------------------

        public async Task Run(Character character, float duration)
        {
            ...
            this.m_RunningLayers[characterId] = layerId;
            
            float startTime = character.Time.Time;
            while (duration > float.Epsilon && character.Time.Time - startTime <= duration)
            {
                if (AsyncManager.ExitRequest) return;
                if (this.m_RunningLayers[characterId] != layerId) return;
                
                float t = (character.Time.Time - startTime) / duration;
                this.Update(character, t);
                
                await Task.Yield();
            }
            
            this.Update(character, 1f);
            this.m_RunningLayers.Remove(characterId);
        }
        
        // PRIVATE METHODS: -----------------------------------------------------------------------
        
        private void Update(Character character, float t)
        {
